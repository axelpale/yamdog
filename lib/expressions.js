...
exports.callSignature = (earmark, namemap) => {
  ...
  // Names, only the short ones
  const shorts = Object.keys(namemap)
  // Pick also the full variants, if unique.
  const fulls = Object.values(namemap).filter(n => !shorts.includes(n))
  const names = fulls.concat(shorts)
  // Give longer names precedence by sorting.
