...
module.exports = (lines) => {
  ...
  lines.forEach((line) => {
    if (activePreTag) {
      ...
      const result = preformatted.findEndingPreTag(line, activePreTag)
      if (result) {
        // Detect if the line after the ending tag begins
        // a new preformatted section. Returns null if not found.
        activePreTag = preformatted.findStartingPreTag(result.rest)
      }
    } else {
      // Detect if a preformatted section begins.
      // Null if no tag found.
      activePreTag = preformatted.findStartingPreTag(line)
    }
